...
const ProjectModal = ({ project, onClose }: ProjectModalProps) => (
  ...
  >
    ...
    <motion.div
      className="relative glass-card rounded-2xl max-w-2xl w-full overflow-hidden max-h-[90vh] overflow-y-auto"
    ...
    >
      ...
      <div className="p-6 sm:p-8">
        ...
        <p className="text-muted-foreground text-sm sm:text-base mb-6">{project.description}</p>
        ...
        <div className="flex flex-col sm:flex-row gap-4">
          <a href={project.link} className="hero-button-primary text-sm text-center">
            View Live
          </a>
          <a href={project.github} className="hero-button-secondary text-sm text-center">
            View Code
          </a>
        </div>
      </div>
      <button
        onClick={onClose}
        className="absolute top-2 right-2 sm:top-4 sm:right-4 w-8 h-8 sm:w-10 sm:h-10 rounded-full glass-card flex items-center justify-center text-foreground hover:text-primary transition-colors z-10"
      >
        ✕
      </button>
    </motion.div>
